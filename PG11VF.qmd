...
# Simulamos los datos de ejemplo (adaptar a tu caso)
set.seed(123)
DF_1 <- tibble(
  genero = factor(sample(c("Masculino", "Femenino"), 100, replace = TRUE)),
  uso_redes = factor(sample(c("Bajo", "Medio", "Alto"), 100, replace = TRUE), ordered = TRUE),
  horas_suenio = rnorm(100, mean = 6.5, sd = 1),
  promedio = rnorm(100, mean = 13, sd = 2)
)

# Revisar estructura de los datos
str(DF_1)

# Ver tabla de frecuencia de variables categóricas
table(DF_1$genero)
table(DF_1$uso_redes)

# Visualizar uso de redes por género
ggplot(DF_1, aes(x = uso_redes, fill = genero)) +
  geom_bar(position = "dodge") +
  labs(title = "Uso de redes sociales por género", x = "Nivel de uso de redes", y = "Frecuencia")

# Comparar promedio académico según uso de redes (boxplot)
ggplot(DF_1, aes(x = uso_redes, y = promedio, fill = uso_redes)) +
  geom_boxplot() +
  labs(title = "Promedio académico según uso de redes sociales", x = "Uso de redes", y = "Promedio")

# Análisis estadístico
anova_result <- aov(promedio ~ uso_redes, data = DF_1)
